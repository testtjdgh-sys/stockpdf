...
import { openDb, findReports, getRecentStocks } from "../index/db";
import { crawlReports } from "../cli/crawl";
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  if (req.method === "POST" && url.pathname === "/crawl") {
    ...
    const reports = findReports(db, { stockQuery, from, to });
    const recentStocks = getRecentStocks(db);
    db.close();
    return new Response(
      renderSearchPage({
        reports: reports.map((row: any) => ({
          id: row.id,
